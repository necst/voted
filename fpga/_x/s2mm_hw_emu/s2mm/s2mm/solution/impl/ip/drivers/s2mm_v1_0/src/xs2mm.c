...
void XS2mm_Set_output_r(XS2mm *InstancePtr, u64 Data) {
    Xil_AssertVoid(InstancePtr != NULL);
    Xil_AssertVoid(InstancePtr->IsReady == XIL_COMPONENT_IS_READY);

    XS2mm_WriteReg(InstancePtr->Control_BaseAddress, XS2MM_CONTROL_ADDR_OUTPUT_R_DATA, (u32)(Data));
    XS2mm_WriteReg(InstancePtr->Control_BaseAddress, XS2MM_CONTROL_ADDR_OUTPUT_R_DATA + 4, (u32)(Data >> 32));
}

u64 XS2mm_Get_output_r(XS2mm *InstancePtr) {
    u64 Data;

    Xil_AssertNonvoid(InstancePtr != NULL);
    Xil_AssertNonvoid(InstancePtr->IsReady == XIL_COMPONENT_IS_READY);

    Data = XS2mm_ReadReg(InstancePtr->Control_BaseAddress, XS2MM_CONTROL_ADDR_OUTPUT_R_DATA);
    Data += (u64)XS2mm_ReadReg(InstancePtr->Control_BaseAddress, XS2MM_CONTROL_ADDR_OUTPUT_R_DATA + 4) << 32;
    return Data;
}
